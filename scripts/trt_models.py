...
def get_path(version, pipeline, controlnet):
...
class BaseModel():
    def __init__(self,
        version='1.5',
        pipeline=PIPELINE_TYPE.CONTROLNET,
        hf_token='',
        device='cuda',
        verbose=True,
        fp16=False,
...
class ControlNet(BaseModel):
    def __init__(self, 
        ...
        super().__init__(version=version, fp16=fp16, device=device, verbose=verbose, max_batch_size=max_batch_size, text_maxlen=text_maxlen, embedding_dim=get_unet_embedding_dim(version, pipeline))
...
def make_ControlNet(version='1.5', pipeline=PIPELINE_TYPE.CONTROLNET, hf_token='', verbose=True, max_batch_size=4, controlnet='depth', lora_weights=None, lora_scale=1):
    return ControlNet(
            pipeline = PIPELINE_TYPE.CONTROLNET,
...
def make_UNetC(version='1.5', pipeline=PIPELINE_TYPE.CONTROLNET, hf_token='', verbose=True, max_batch_size=4, controlnet='depth', lora_weights=None, lora_scale=1):
    return UNetC(version=version, pipeline=pipeline, hf_token=hf_token, fp16=True, device='cuda', verbose=verbose,
