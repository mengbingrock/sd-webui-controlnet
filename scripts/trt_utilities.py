...
class Engine():
    ...
    def infer(self, feed_dict, stream, use_cuda_graph=False):
        ...
        for name, buf in feed_dict.items():
            # print(self.engine_path,'!!! check tensor name and shapes:' , name, (self.tensors[name].shape , buf.shape) if hasattr(self.tensors[name], 'shape') else (self.tensors[name], buf))
            input(f'check iotensors {name}')
